...
const InputField = ({type, placeholder, name, handleChange, address})=>(
    ...
    name={name}
    // This value prop is the key. It needs to find a matching key in the 'address' state object.
    value={address[name]} 
    required
    />
)
...
const AddAddress = () => {
    ...
    const [address , setAddress] = useState({ // Renamed to setAddress for clarity
        firstName: '',
        lastName: '', // This must match the 'name' prop exactly
        email: '',
        street: '',
        city: '',
        state: '',
    ...
    const handleChange = (e)=>{
        ...
        setAddress((prevAddress)=>({ // Using the clearer name
            ...prevAddress,
            [name]: value,
        }))
    }
        ...
        try {
            const {data} = await axios.post('/api/address/add', {address});
             if(data.success){
                ...
                navigate('/cart') // Redirect to cart after adding address
             }else{
                // Use data.message from the server response
         ...
         <div className='flex flex-col-reverse md:flex-row justify-between mt-10'>
            ...
            <form onSubmit={onSubmitHandler}  className='space-y-3 mt-6 text-sm'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                    ...
                    {/* ✅ FIX: Changed 'LastName' to 'lastName' to match the state and schema */}
                    <InputField handleChange={handleChange} address={address}
                ...
                {/* Wrapped these in a div to prevent layout issues */}
                <div className='grid grid-cols-2 gap-4'>
